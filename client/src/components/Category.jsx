    ...
    <div className='sm:w-[90%] sm:ms-[5%] max-sm:px-3 pt-5'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4'>
                {categories.map((item,index)=>(
                  <div key={index} className='px-4 py-8 group hover:scale-105 duration-300 flex flex-col items-center justify-center rounded-md cursor-pointer' style={{backgroundColor: item.bgColor}} onClick={()=>{navigate(`/product/${item.path.toLowerCase()}`);scrollTo(0,0)}}>
