    ...
    <div className="flex flex-col justify-between bg-transparent trxt-black dark:text-white">
            <form className="md:p-10 p-4 space-y-5 max-w-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                <input accept="image/*" type="file" id={`image${index}`} hidden required />
                ...
                <div className="flex flex-col gap-1 max-w-md">
                    ...
                    <textarea id="product-description" rows={4} className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40 resize-none" placeholder="Type here" required></textarea>
                </div>
                ...
                <button className="px-8 py-2.5 bg-[#008000ba] hover:bg-[green] duration-300 w-full tracking-[3px] text-white font-medium rounded-md" onClick={onClickHandler}>ADD</button>
            </form>
        </div>
  )
}
